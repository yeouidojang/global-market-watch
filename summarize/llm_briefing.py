...
def _fmt_stocks_asia(stocks_data: dict) -> str:
    ...
    def _row_ov(div, s):
        ...
        return (f"| {div} | {s['ticker']} | {s['name'][:18]} | {sec} | "
                ...
                f"{_r(s.get('ret_1m'))} | {_tv_ov(s)} | {_r(s.get('tv_chg'),'+.0f')} | {note} |")
...
국가별(일본→중국→홍콩 순)로 분리해 각각 아래 항목을 서술하세요.

**공통 분석 구조 (국가별 반복)**
  - 시장 폭: 상승비율·ADR 20일 누적으로 광범위 상승 vs 소수 집중 판단
    (ADR >60% 광범위 강세 / 50~60% 중립∼완만한 강세 / <40% 광범위 약세)
  - 섹터 분석: 강세/약세 섹터 상위 2~3개 (시총가중 등락률 기준), 업종 방향성 1줄
  - 대형주(시총·거래대금 상위): 주도 종목의 테마 및 수급 집중 여부
  - 특징주: 비고(signal)의 급등락 배경과 테마 분류 (반도체·전기차·금융·부동산 등)

**시장별 핵심 드라이버**
  - 일본(Nikkei225): 엔/달러 방향과 수출주(반도체·자동차) 연동, 반도체 장비·전자부품 흐름
  - 중국(CSI300): 정책 금리·유동성 변화, IT 규제 여부, 부동산 섹터 약세 지속 판단
  - 홍콩(HSI): 위안화 추이, 중국 본토 자본 유입/유출, 부동산·금융 투매 여부

**해외 → 한국 전이 테마** (마지막에 국가별로 명시)
  - 일본 강세 섹터/종목 → KOSPI/KOSDAQ 연관 업종·종목 연결
    (예: 일본 반도체 장비 강세 → 한미반도체·원익IPS / 일본 자동차 강세 → 현대차·기아)
  - 중국 강세/약세 → 한국 중국 수혜·피해 업종 연결
    (예: 중국 IT 강세 → 게임·플랫폼주 / 중국 부동산 약세 → 한국 건자재·화학 주의)
  - 홍콩 흐름 → 한국 외국인 수급 방향 시사점
...
(강세/약세 섹터 — 표로 작성, 2~3개 집중. DAX/FTSE/CAC 시장 폭 데이터를 참고해 광범위 상승 vs 소수 집중 여부 1문장 분석.
 ADR 해석: >1.5 광범위 강세 / 1.0~1.5 중립∼완만한 강세 / <0.7 광범위 약세)

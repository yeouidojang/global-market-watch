...
**핵심 요약** (3~4문장, 미국·유럽·달러·원자재 핵심 드라이버 중심. 한국 언급 없이 글로벌 흐름만)

---

## 핵심 매크로 드라이버 (매일 고정)
(당일 시장 전체를 움직인 핵심 변수 3개:
| 드라이버 | 방향 | 시사점 |
|---------|------|--------|
...
(아래 관점으로 분석하세요:
  ...
  - VIX·DXY 흐름이 신흥국 자금 흐름에 주는 글로벌 매크로 의의)

---

{'## 유럽 시장 상세 — DAX · FTSE100 · CAC40' if has_europe else ''}
{'(stocks의 "유럽 시장 데이터" 섹션을 활용해 독일·영국·프랑스 3개 시장을 각각 분리해서 분석하세요.' if has_europe else ''}
{'Asia 브리핑에서 일본·중국·홍콩을 각각 분석하듯, 아래 형식으로 3개 시장 각각 작성:' if has_europe else ''}
{'' if has_europe else ''}
{'### DAX (독일)' if has_europe else ''}
{'- 주요 섹터 강약 (STOXX600 섹터 중 독일 비중 큰 자동차·화학·산업재·금융 중심)' if has_europe else ''}
{'- 시총 상위 대형주 동향 (종목·종가·등락률·거래대금 표)' if has_europe else ''}
{'- 거래대금 급증 특징주 및 테마' if has_europe else ''}
{'' if has_europe else ''}
{'### FTSE100 (영국)' if has_europe else ''}
{'- 주요 섹터 강약 (에너지·금융·헬스케어·광산 중심)' if has_europe else ''}
{'- 시총 상위 대형주 동향' if has_europe else ''}
{'- 거래대금 급증 특징주 및 테마' if has_europe else ''}
{'' if has_europe else ''}
{'### CAC40 (프랑스)' if has_europe else ''}
{'- 주요 섹터 강약 (럭셔리·항공우주·에너지·금융 중심)' if has_europe else ''}
{'- 시총 상위 대형주 동향' if has_europe else ''}
{'- 거래대금 급증 특징주 및 테마' if has_europe else ''}
{'' if has_europe else ''}
{'※ 종목이 어느 지수 소속인지 "index" 컬럼을 활용해 DAX/FTSE/CAC 별로 분류해 작성)' if has_europe else ''}
...
## 주요 경제지표 — 이번주·다음주
(FOMC·고용·CPI·PMI 등 US/EU/JP/KR/CN — 발표일·국가·지표명·예상치·실제치를 표로 작성.
이미 발표된 항목은 실제치를 포함하고 서프라이즈 여부를 1문장 코멘트)

---

## SPX 기업실적 — 이번주·다음주
(발표일·티커·시간대(BMO/AMC)·EPS 예상/실제·서프라이즈(%)를 표로 작성.
이미 발표된 종목은 결과를 포함하고, 향후 주목할 대형주 1~2개를 1문장 코멘트)

---

## 시사점 + 아시아 증시 주목 포인트
(미국·유럽 흐름을 종합해 아시아 개장 시 주목할 포인트를 정리하세요:
  - **미국 → 아시아 전이 테마**: 미국 반도체 강세 → 한국 SK하이닉스·삼성전자 / 미국 전기차 → 2차전지·소재 / 미국 빅테크 → 한국 IT부품 / 미국 에너지 강세 → 한국 정유 등 구체적으로 연결
  - **유럽 → 아시아 전이 테마**: ASML·자동차·럭셔리·헬스케어 등 유럽 섹터와 한국·일본·중국 연관 종목 연결
  - **매크로 경계 포인트**: DXY·VIX·금리 방향이 한국·아시아 외국인 수급에 주는 영향
